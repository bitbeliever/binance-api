...
func UpdateMarginType(symbol string, typ futures.MarginType) error {
	return client.NewClient().NewChangeMarginTypeService().Symbol(symbol).MarginType(typ).Do(context.Background())
}
...
func ModifyLeverage(symbol string, leverage int) (*futures.SymbolLeverage, error) {
	return client.NewClient().NewChangeLeverageService().Symbol(symbol).Leverage(leverage).Do(context.Background())
}

// LeverageBracket 杠杆分层标准 /fapi/v1/leverageBracket
func LeverageBracket(symbol string) ([]*futures.LeverageBracket, error) {
	return client.NewClient().NewGetLeverageBracketService().Symbol(symbol).Do(context.Background())
}
...
func PositionMode() (*futures.PositionMode, error) {
	return client.NewClient().NewGetPositionModeService().Do(context.Background())
}

// PositionModeChange 更改持仓模式
func PositionModeChange(dual bool) error {
	return client.NewClient().NewChangePositionModeService().DualSide(dual).Do(context.Background())
}
